...
func TestAuditWorker_HandleEventMessage(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "Handle QUEUE_CANCELLED by patient",
			...
			expectedAction: "QUEUE_CANCELLED",
			expectedRole:   string(domain.RolePatient),
			expectedActor:  "Alice Smith",
			expectedIP:     "127.0.0.1",
			expectRecord:   true,
		...
		{
			name:  "Handle QUEUE_CANCELLED default patient without role or name",
			event: "QUEUE_CANCELLED",
			data: map[string]any{
				"queue_number": "A-02",
			},
			expectedAction: "QUEUE_CANCELLED",
			expectedRole:   string(domain.RolePatient),
			expectedActor:  "Patient",
			expectedIP:     "127.0.0.1",
			expectRecord:   true,
		},
		{
			name:  "Handle QUEUE_CANCELLED by admin on behalf of patient",
			event: "QUEUE_CANCELLED",
			data: map[string]any{
				"patient_name": "Alice Smith",
				"user_id":      float64(10),
				"cancelled_by": float64(99),
				"queue_number": "A-01",
