...
type QueueUseCase interface {
	...
	JoinQueue(ctx context.Context, userID *int, patientName string) (*domain.QueueTicket, error)

	// GetMyTicket retrieves the active ticket for a logged-in patient with recalculated wait times.
	GetMyTicket(ctx context.Context, userID int) (*domain.QueueTicket, error)
