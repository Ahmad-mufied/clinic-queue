...
type ConsultationRepositoryPort interface {
	...
	CallNextTicketAtomically(ctx context.Context, doctorID int) (*domain.ConsultationSession, error)

	// FinishActiveSession completes the active consultation session and marks the corresponding ticket as COMPLETED.
	FinishActiveSession(ctx context.Context, doctorID int) (*domain.ConsultationSession, error)
