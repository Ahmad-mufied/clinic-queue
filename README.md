...
- **Authentication & RBAC**: Secure login and role-based access control (Casbin) for Patients, Doctors, and Admins.
- **Patient Queue Engine**: Online walk-in registration with real-time SSE queue monitoring and dynamic wait estimations.
- **Doctor Workspace**: Seamless shift management (Online/Offline) and consultation workflow (call next patient, complete session).
...
2. **Run Backend (Go)**
   ```bash
   go mod download
   # Run with Air for hot-reloading (if configured) or standard go run
