...
export default function AdminAuditTrailPage() {
  ...
  const handleCopyReqID = (reqId: string) => {
    navigator.clipboard.writeText(reqId);
    setReqIdCopied(true);
    setTimeout(() => setReqIdCopied(false), 2000);
  };
            ...
            <>
              <DialogHeader className="space-y-3 pr-6">
                <div className="flex flex-wrap items-center gap-2.5">
                  <div className="inline-flex items-center gap-1.5 text-xs font-mono font-bold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800/80 pl-3 pr-2 py-1.5 rounded-xl border border-slate-200/80 dark:border-slate-700">
                    <Fingerprint className="h-3.5 w-3.5 text-emerald-600 shrink-0" />
                    <span className="text-slate-400 font-semibold uppercase text-[10px] tracking-wider">Log ID:</span>
                    <span className="select-all">{inspectLog.id}</span>
                    <Button
                      onClick={() => handleCopyLogID(inspectLog.id)}
                      variant="ghost"
                      size="sm"
                      className="h-6 px-1.5 ml-1 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 cursor-pointer"
                      title="Copy Log ID"
                    >
                      {logIdCopied ? (
                  ...
                  <span className="font-mono font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200/70 dark:border-emerald-800/60 px-3 py-1.5 rounded-full text-xs">
                    {inspectLog.action}
                  </span>
                  <div className="flex items-center gap-1.5 text-xs text-slate-400 font-mono sm:ml-auto">
                ...
                <div>
                  ...
                  <DialogDescription className="sr-only">
                    Audit Activity Inspector Details
                  </DialogDescription>
                </div>
              </DialogHeader>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-800 space-y-1.5">
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="h-7 w-7 rounded-full bg-emerald-600 text-white flex items-center justify-center text-[11px] font-bold shrink-0 shadow-xs">
                      {inspectLog.actor_name?.charAt(0) || "U"}
                    </div>
                    <span className="text-xs font-bold text-slate-900 dark:text-white leading-tight break-words" title={inspectLog.actor_name}>
                ...
                <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-800 space-y-1.5">
                  ...
                  <div>
                    <span
                      className={`inline-block text-[11px] uppercase font-bold px-3 py-0.5 rounded-full ${
                ...
                {(() => {
                  ...
                  return (
                    <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-800 space-y-1.5">
                      ...
                      <span className="text-xs font-mono font-bold text-emerald-700 dark:text-emerald-400 block break-words" title={identity.handle}>
                        {identity.handle}
                      </span>
                    </div>
                  );
              ...
              <div className="space-y-2">
                ...
                {/* Clean Light Mode Code Frame */}
                <div className="rounded-2xl bg-slate-50 border border-slate-200/90 overflow-hidden shadow-xs">
                  <div className="flex items-center justify-between px-4 py-2.5 bg-slate-100/90 border-b border-slate-200/90 text-slate-500 text-xs">
